...
// URL patterns (Vercel passes full path via req.url):
//   GET    /api/assets                → list
//   POST   /api/assets                → create
//   GET    /api/assets/[num]          → single
//   PUT    /api/assets/[num]          → update
//   DELETE /api/assets/[num]          → delete (admin)
//   GET    /api/assets/[num]/scans    → scan history
//   POST   /api/assets/[num]/scans    → record scan
...
module.exports = async (req, res) => {
  ...
  // Parse path: /api/assets  or  /api/assets/AST-001  or  /api/assets/AST-001/scans
  const url      = req.url.split('?')[0].replace(/\/$/, '');
  const parts    = url.replace(/^\/api\/assets\/?/, '').split('/').filter(Boolean);
  const assetNum = parts[0] || null;
  const sub      = parts[1] || null;   // 'scans' or null
  ...
  if (req.method === 'GET') {
    const { rows:[a] } = await query(
      `SELECT a.*,u.full_name AS assigned_user_name,u.email AS assigned_user_email
       FROM assets a LEFT JOIN users u ON a.assigned_to=u.id
       WHERE a.asset_number=$1`, [assetNum]
    );
