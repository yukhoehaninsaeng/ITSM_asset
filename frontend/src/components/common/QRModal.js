...
export default function QRModal({ asset, onClose }) {
  ...
  const download = () => {
    ...
    a.href = qrUrl;
    a.download = `QR_${asset.asset_number}.png`;
    a.click();
  };
  ...
  const print = () => {
    ...
    w.document.write(`
      ...
      <style>
        ...
        h2{margin:16px 0 4px;font-size:17px}
      ...
      <div className="modal" style={{ maxWidth: 400 }}>
        ...
        <div className="modal-bd" style={{ textAlign: 'center' }}>
          ...
          <div style={{
            ...
            {loading ? (
              <div style={{ width: 220, height: 220, background: '#f0f2f4', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-3)' }}>
                생성 중...
              </div>
            ) : qrUrl ? (
          ...
          <div className="form-group" style={{ textAlign: 'left', marginBottom: 4 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              />
              <button className="btn btn-ghost btn-sm" style={{ flexShrink: 0 }} onClick={() => generate(baseUrl)}>
                재생성
              </button>
            </div>
          </div>
          <p style={{ fontSize: 11, color: 'var(--text-3)', textAlign: 'left', marginTop: 4 }}>
            스캔 URL: {baseUrl}/scan/{asset.asset_number}
          </p>
        </div>
