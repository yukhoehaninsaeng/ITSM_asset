...
export default function AssetDetail() {
  ...
  const load = async () => {
    try {
      const [a, s] = await Promise.all([assetsAPI.get(assetNumber), assetsAPI.scans(assetNumber)]);
      setAsset(a.data); setScans(s.data);
    } catch { toast.error('자산을 찾을 수 없습니다'); navigate('/assets'); }
    finally { setLoading(false); }
  };
  useEffect(() => { load(); }, [assetNumber]); // eslint-disable-line

  if (loading) return <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'60vh', color:'var(--text-3)' }}>불러오는 중...</div>;
  ...
  return (
    <div style={{ padding:'24px 28px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:18, fontSize:13, color:'var(--text-3)' }}>
        ...
        <span>›</span><span>{asset.asset_number}</span>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 360px', gap:16, alignItems:'start' }} className="detail-grid">
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div className="card"><div className="card-header"><span className="card-title">기본 정보</span></div><div className="card-body">
            <Row label="카테고리" value={CL[asset.category]||asset.category} />
            <Row label="제조사" value={asset.manufacturer} /><Row label="모델" value={asset.model} />
            <Row label="시리얼 번호" value={asset.serial_number} />
            <Row label="구매일" value={fmt(asset.purchase_date)} /><Row label="보증 만료일" value={fmt(asset.warranty_expiry)} />
            <Row label="설명" value={asset.description} />
          </div></div>
          <div className="card"><div className="card-header"><span className="card-title">위치 및 배정</span></div><div className="card-body">
            <Row label="위치" value={asset.location} /><Row label="부서" value={asset.department} />
            <Row label="담당자" value={asset.assigned_user_name} /><Row label="담당자 이메일" value={asset.assigned_user_email} />
            <Row label="비고" value={asset.notes} />
          </div></div>
        </div>
        <div className="card" style={{ position:'sticky', top:24 }}>
          <div className="card-header"><span className="card-title">스캔 이력</span><span style={{ fontSize:12, color:'var(--text-3)' }}>{scans.length}건</span></div>
          {scans.length===0 ? <div style={{ padding:32, textAlign:'center', color:'var(--text-3)', fontSize:13 }}>스캔 기록 없음</div> : (
