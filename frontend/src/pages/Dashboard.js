...
export default function Dashboard() {
  ...
  if (!stats) return <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'60vh', color:'var(--text-3)' }}>불러오는 중...</div>;

  const pieData = [
    { name:'사용 중', value:+stats.in_use },{ name:'재고', value:+stats.in_stock },
    { name:'유지보수', value:+stats.maintenance },{ name:'폐기', value:+stats.retired },{ name:'분실', value:+stats.lost },
  ].filter(d => d.value > 0);
  const catData = Object.entries(stats.category_breakdown||{}).map(([name,value]) => ({ name, value:+value }));

  return (
    <div style={{ padding:'24px 28px' }}>
      <div style={{ marginBottom:22 }}><h1 style={{ fontSize:21, fontWeight:700 }}>대시보드</h1><p style={{ color:'var(--text-2)', fontSize:13, marginTop:2 }}>자산 현황 요약</p></div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ height:230 }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart><Pie data={pieData} cx="50%" cy="45%" innerRadius={52} outerRadius={85} paddingAngle={3} dataKey="value">{pieData.map((_,i) => <Cell key={i} fill={Object.values(SC)[i]} />)}</Pie><Tooltip formatter={(v,n) => [`${v}개`,n]} /></PieChart>
            </ResponsiveContainer>
            <div style={{ display:'flex', flexWrap:'wrap', gap:'4px 12px', justifyContent:'center', marginTop:-6 }}>
              {pieData.map((d,i) => <span key={d.name} style={{ fontSize:11, color:'var(--text-2)', display:'flex', alignItems:'center', gap:4 }}><span style={{ width:8, height:8, borderRadius:'50%', background:Object.values(SC)[i], display:'inline-block' }} />{d.name} ({d.value})</span>)}
            </div>
          </div>
        </div>
        <div className="card">
          ...
          <div className="card-body" style={{ height:230 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={catData} margin={{ top:4, right:8, left:-22, bottom:0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#eee" /><XAxis dataKey="name" tick={{ fontSize:11 }} /><YAxis tick={{ fontSize:11 }} allowDecimals={false} /><Tooltip />
              ...
              <tbody>
                {stats.recent_scans.map(s => (
                  <tr key={s.id}>
                    ...
                    <td style={{ color:'var(--text-3)', fontSize:12 }}>{formatDistanceToNow(new Date(s.scan_time),{ addSuffix:true, locale:ko })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
