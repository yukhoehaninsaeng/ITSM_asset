      ...
      <nav style={{ flex: 1, padding: '10px 8px' }}>
        ...
        {user?.is_admin && (
          <>
            ...
            <NavLink to="/users" onClick={onNav}
              style={({ isActive }) => navStyle(isActive)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ color: 'rgba(255,255,255,.38)', fontSize: 11 }}>
              {user?.is_admin ? '관리자' : (user?.department || '사용자')}
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <header style={{
        ...
        height: 56, background: 'var(--sidebar-bg)', zIndex: 200,
      ...
      {open && (
        <>
          ...
          <aside style={{
            ...
            background: 'var(--sidebar-bg)', zIndex: 400, display: 'flex', flexDirection: 'column',
          }}>
      ...
      {/* Main */}
      <main style={{ flex: 1, marginLeft: 'var(--sidebar-w)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        {children}
      </main>

      <style>{`
        @media (min-width: 769px) { .mobile-topbar { display: none !important; } .desktop-sidebar { display: flex !important; } }
        @media (max-width: 768px) { .desktop-sidebar { display: none !important; } .mobile-topbar { display: flex !important; } main { margin-left: 0 !important; padding-top: 56px; } }
      `}</style>
    </div>
  );
}
