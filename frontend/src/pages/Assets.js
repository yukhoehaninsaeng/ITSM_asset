...
export default function Assets() {
  ...
  const del = async (num) => {
    ...
    try { await assetsAPI.delete(num); toast.success('삭제되었습니다'); load(); }
    catch (err) { toast.error(err.response?.data?.error || '삭제 실패'); }
  };

  const start = Math.max(1, Math.min(page-2, data.total_pages-4));
  const pageNums = Array.from({ length:Math.min(5, data.total_pages) }, (_,i) => start+i);

  return (
    <div style={{ padding:'24px 28px' }}>
      <div className="flex-between" style={{ marginBottom:22, flexWrap:'wrap', gap:12 }}>
        <div><h1 style={{ fontSize:21, fontWeight:700 }}>자산 목록</h1><p style={{ color:'var(--text-2)', fontSize:13, marginTop:2 }}>총 {data.total}개</p></div>
        <button className="btn btn-primary" onClick={() => { setEditAsset(null); setShowModal(true); }}>+ 자산 등록</button>
      </div>
      <div className="card" style={{ marginBottom:14 }}>
        <div style={{ padding:'12px 16px', display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
          <input className="form-control" style={{ flex:'1 1 220px', maxWidth:300 }} placeholder="자산번호, 이름, 시리얼..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
          <select className="form-control" style={{ width:140 }} value={fStatus} onChange={e => { setFStatus(e.target.value); setPage(1); }}>
            <option value="">모든 상태</option>{Object.entries(SL).map(([v,l]) => <option key={v} value={v}>{l}</option>)}
          </select>
          <select className="form-control" style={{ width:140 }} value={fCat} onChange={e => { setFCat(e.target.value); setPage(1); }}>
            <option value="">모든 카테고리</option>{Object.entries(CL).map(([v,l]) => <option key={v} value={v}>{l}</option>)}
          </select>
          {(search||fStatus||fCat) && <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setFStatus(''); setFCat(''); setPage(1); }}>초기화</button>}
        </div>
      </div>
      <div className="card">
        <div className="tbl-wrap">
          {loading ? <div style={{ padding:48, textAlign:'center', color:'var(--text-3)' }}>불러오는 중...</div>
          : data.items.length===0 ? <div style={{ padding:48, textAlign:'center', color:'var(--text-3)' }}><div style={{ fontSize:40, marginBottom:8 }}>⊡</div>자산이 없습니다</div>
          : (
            <table>
              <thead><tr><th>자산번호</th><th>이름</th><th>카테고리</th><th>상태</th><th className="hide-mobile">위치</th><th className="hide-mobile">부서</th><th style={{ width:120 }}>작업</th></tr></thead>
              <tbody>
                {data.items.map(a => (
                  <tr key={a.id}>
                    <td><span className="mono">{a.asset_number}</span></td>
                    <td><button onClick={() => navigate(`/assets/${a.asset_number}`)} style={{ background:'none', border:'none', cursor:'pointer', fontWeight:500, color:'var(--primary)', fontFamily:'inherit', fontSize:13, padding:0, textAlign:'left' }}>{a.name}</button></td>
        ...
        {data.total_pages > 1 && (
          <div style={{ padding:'12px 20px', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <div style={{ display:'flex', gap:4 }}>
              ...
              {pageNums.map(n => <button key={n} className="btn btn-sm" onClick={() => setPage(n)} style={{ background:n===page?'var(--primary)':'transparent', color:n===page?'#fff':'var(--text-2)', border:'1px solid var(--border)' }}>{n}</button>)}
