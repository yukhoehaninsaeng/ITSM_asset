...
export default function Scan() {
  ...
  const startScanner = () => {
    ...
    setResult(null);
    setTimeout(() => {
      const s = new Html5QrcodeScanner('qr-reader', {
        fps: 10,
      ...
      s.render(
        async (text) => { s.clear().catch(() => {}); setScanning(false); await lookup(extractAssetNumber(text)); },
        () => {}
      );
  ...
  const extractAssetNumber = (url) => {
    const m = url.match(/\/scan\/([^/?#\s]+)/);
  ...
  const lookup = async (num) => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        setResult({ found: false, asset_number: num });
      } else {
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 600, margin: '0 auto' }}>
      <div style={{ marginBottom: 22 }}>
        <h1 style={{ fontSize: 21, fontWeight: 700 }}>QR 스캔</h1>
        <p style={{ color: 'var(--text-2)', fontSize: 13, marginTop: 2 }}>카메라로 스캔하거나 자산번호를 직접 입력하세요</p>
      </div>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        ...
        <div className="card-body">
          {!scanning ? (
            <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', padding: 13, fontSize: 14 }} onClick={startScanner}>
              📷 카메라 스캔 시작
            </button>
          ) : (
            <>
              <div id="qr-reader" />
              <button className="btn btn-ghost" style={{ width: '100%', marginTop: 10, justifyContent: 'center' }} onClick={stopScanner}>
                스캔 중지
              </button>
            </>
          )}
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        ...
        <div className="card-body">
          <form onSubmit={e => { e.preventDefault(); lookup(manual); }} style={{ display: 'flex', gap: 8 }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading || !manual}>
              {loading ? '...' : '조회'}
            </button>
          </form>
        </div>
      ...
      {result && (
        <div className="card" style={{ borderLeft: `4px solid ${result.found ? 'var(--success)' : 'var(--danger)'}` }}>
          <div className="card-header">
            <span className="card-title">{result.found ? '✅ 자산 확인됨' : '❌ 미등록 자산'}</span>
          </div>
          <div className="card-body">
            {result.found ? (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 24px', marginBottom: 18 }}>
                  {[
                    ['자산번호', <span className="mono">{result.asset_number}</span>],
                    ['자산명', result.name],
                    ['상태', <span className={SB[result.status]}>{SL[result.status] || result.status}</span>],
                    ['카테고리', result.category],
                    ['위치', result.location || '—'],
                    ['부서', result.department || '—'],
                  ].map(([label, val]) => (
                    <div key={label}>
                      <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '.04em' }}>{label}</div>
              ...
              <div style={{ textAlign: 'center', padding: '8px 0' }}>
                ...
                <p style={{ color: 'var(--text-2)' }}>
                  <strong>{result.asset_number}</strong>은 등록되지 않은 자산입니다
                </p>
                <button className="btn btn-primary" style={{ marginTop: 14 }}
                  onClick={() => navigate('/assets')}>자산 등록하기</button>
              </div>
            )}
          </div>
        </div>
